...
name.file <- 'ejemplo2_paper_2018_500.individal_lognormal'
...
output.dir <- paste0("output/", name.file, "/individuals/SME")
densi_SME  <- read.table(file =  paste0(output.dir, '/density_maxent_severity', nrow(S), '.txt')) %>% mutate(name = 'SME')
output.dir <- paste0("output/", name.file, "/individuals/SMEE2")
densi_SMEE  <- read.table(file =  paste0(output.dir, '/density_maxent_severity', nrow(S), '.txt'))  %>% mutate(name = 'SMEE')
output.dir <- paste0("output/", name.file, "/individuals/MEM_Poisson")
densi_MEM_Poisson  <- read.table(file =  paste0(output.dir, '/density_maxent_severity', nrow(S), '.txt'))  %>% mutate(name = 'MEM Poisson')
...
densi_gg
dir <- paste0(geted(), '/1.-TEXTO_PAPER/NUMERICAL_CASE_PAPER/')
if(!is.null(dir)) ggsave(paste0(dir, '/individual_density_comparison', nrow(S), '.png'))
...
dir <- getwd('/1.-TEXTO_PAPER/NUMERICAL_CASE_PAPER/')
if(!is.null(dir)) ggsave(paste0(dir, '/individual_calibration_diagram_', nrow(S), '.png'))
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
